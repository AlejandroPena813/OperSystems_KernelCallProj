...
//int runToReady;//project 2 variable for Run2Rdy
buff procArr[1024];
...
int ex_time(){
	...
	int x;
	x = sys_sample(5678);
	//printf("value inEXIT from kernel: %d\n", x);
	//printf("runToREady %d\n", runToReady);
...
if(buffToggle == 1){
	...
	if(x == 4){
		procArr[buffcounter].boolRdyRun = 1;
		procArr[buffcounter].boolRunBlock = 1;
	}
	else if(x == 3){
		procArr[buffcounter].boolRdyRun =1 ;
		procArr[buffcounter].boolRunBlock =0 ;
	}
	else if(x == 2){
		procArr[buffcounter].boolRdyRun =0 ;
		procArr[buffcounter].boolRunBlock =1 ;
	}
	else{ //x == 0
		procArr[buffcounter].boolRdyRun =0 ;
...
int j;//loop counter
	...
	for(j = 0; j< buffSize; j=j+1){
		if(procArr[j].childID == inputPID){
			...
			printf("Process intitialized and into Ready state \n");
			if((procArr[j].boolRdyRun ==1 )&&(procArr[j]).boolRunBlock == 1){
			printf("Ready to Run \n");
			printf("Run to Blocked\n");
			}
			else if((procArr[j].boolRdyRun ==1 )&&(procArr[j]).boolRunBlock ==0 ){
			printf("Ready to Run\n");
			}
			else if((procArr[j].boolRdyRun ==0 )&&(procArr[j]).boolRunBlock ==1 ){
			printf("Run to Blocked \n");
			}
			else{
			printf("Process did not go from Ready to Run or Run to Blocked \n");
			}
			printf("Process succesfully went into Terminated state \n");//can delete
			return 0;
		}
	}
...
int inputIndex = lookIndex;//storing index value
 ...
 if( (inputIndex >= buffSize) || (inputIndex < 0) )//index out of range
 {
	 ...
	 printf("Process Initialized and into Ready state  \n" );//can delete
	 if((procArr[inputIndex].boolRdyRun == 1)&& (procArr[inputIndex].boolRunBlock ==1 ) ){
		printf("Ready to Run \n");
		printf("Run to Blocked\n");
	 }
	 else if((procArr[inputIndex].boolRdyRun ==1 ) && (procArr[inputIndex].boolRunBlock == 0)){
		printf("Ready to Run\n");
	 }
	 else if((procArr[inputIndex].boolRdyRun ==0 )&&(procArr[inputIndex].boolRunBlock == 1)){
		printf("Run to Blocked\n");
	 }
	 else{
		printf("Process did not go from Ready to Run or Run to Blocked \n");
	 } 
	 printf("Process succesfully went to Terminated state \n");
